...
import (
	...
	"github.com/shadowpr1est/knigapoisk-review-service/config"
	reviewgrpc "github.com/shadowpr1est/knigapoisk-review-service/internal/delivery/grpc"
	"github.com/shadowpr1est/knigapoisk-review-service/internal/logger"
	"github.com/shadowpr1est/knigapoisk-review-service/internal/repository/postgres"
...
func main() {
	...
	logg := logger.WithService(baseLogger, "review-service")
	...
	useCase := review.NewReviewUseCase(reviewRepo, ratingRepo)

	serverLogger := logg.With(zap.String("component", "grpc-server"))

	grpcServer := grpc.NewServer(
		grpc.ChainUnaryInterceptor(
			reviewgrpc.LoggingInterceptor(serverLogger),
			reviewgrpc.RecoveryInterceptor(serverLogger),
		),
	)

	reviewServer := reviewgrpc.NewReviewServer(useCase, serverLogger)

...
import (
	...
	"github.com/shadowpr1est/knigapoisk-file-service/config"
	filegrpc "github.com/shadowpr1est/knigapoisk-file-service/internal/delivery/grpc"
	"github.com/shadowpr1est/knigapoisk-file-service/internal/logger"
	"github.com/shadowpr1est/knigapoisk-file-service/internal/repository/minio"
	"github.com/shadowpr1est/knigapoisk-file-service/internal/repository/postgres"
...
func main() {
	...
	logg := logger.WithService(baseLogger, "file-service")
	...
	storageRepo, err := minio.NewStorage(
		cfg.MinIOEndpoint,
		cfg.MinIOAccessKey,
		cfg.MinIOSecretKey,
		cfg.MinIOBucket,
	...
	if err != nil {
		logg.Fatal("failed to init storage", zap.Error(err))
	}

	useCase := file.NewFileUseCase(fileRepo, storageRepo)

	serverLogger := logg.With(zap.String("component", "grpc-server"))

	grpcServer := grpc.NewServer(
		grpc.ChainUnaryInterceptor(
			filegrpc.LoggingInterceptor(serverLogger),
			filegrpc.RecoveryInterceptor(serverLogger),
		),
	)

	fileServer := filegrpc.NewFileServer(useCase, serverLogger)
